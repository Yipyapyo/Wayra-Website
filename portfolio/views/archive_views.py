        ...
        if request.method == "GET":
            ...
            if(searched == ""):
                ...
                if request.session['archived_individual_filter'] == '2':
                    ...
                    individual_search_result = Individual.objects.filter(id__in=founder_individuals.values('individualFounder'), name__contains=searched, is_archived=True).values().order_by('id')
                elif request.session['archived_individual_filter'] == '3':
                    individual_search_result = InvestorIndividual.objects.filter(name__contains=searched, is_archived=True).values().order_by('id')
                else:
                    individual_search_result = Individual.objects.filter(name__contains=searched, is_archived=True).values().order_by('id')
